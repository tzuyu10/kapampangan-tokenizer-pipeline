...
| 5 — NLLB fine-tune | **first Colab run done, recipe revised, re-run pending**. `experiments/nllb_finetune_v1/` + `notebooks/phase5-nllb-morphbpe-finetune.ipynb`. Conditions: `morphbpe` / `penalty8` vs **`unigram6080`** (fair headline) + `nllb_zeroshot` (reference). **First run: random-init embedding-only collapsed to chrF++ ~10 vs zero-shot 33.6.** Notebook revised 2026-09-03: warm-start the embedding from NLLB's own sub-token means (`vocab.json`), select on dev loss, 1 seed first, ~20 min/run. If warm-start still can't beat zero-shot -> LoRA-on-encoder condition or accept the low-resource null. |
