...
- `backend/tokenizer/artifacts/morphbpe-penalty8/*`, `plain-bpe/*`, and
  `unigram-lm-6080/*` — the exact artifact files copied from your
  `demo/morphbpe-penalty8/`, `demo/plain-bpe/`, and `demo/unigram-lm-6080/`.
  The two BPE artifacts get the same `checksums.sha256` re-hash on load as
  before (existing `tokenizer.py` code) — so if either was ever altered, the
  backend refuses to start rather than silently serving a different
  tokenizer than the one you trained. `unigram-lm-6080` has no
...
- `backend/tokenizer/unigram_lm.py` — the `UnigramLM` class (the Viterbi
  ...
  relative to its own file location (`HERE / "morphbpe-penalty8"` etc.) —
  importing it as-is from a different directory would have tried to load
  artifacts from the wrong place. The class itself is untouched.
