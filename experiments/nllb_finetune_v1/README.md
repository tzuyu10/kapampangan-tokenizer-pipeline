...
## Conditions (each x 3 seeds)

| condition | encoder source tokenizer | trainable params |
|---|---|---|
| `nllb_native` | NLLB SentencePiece (Kapampangan read as `tgl_Latn`) | encoder input embedding, untied from `shared` (256,206 x 1,024) |
| `morphbpe` | paper-aligned hard-constrained MorphBPE @ 6,080 | fresh `nn.Embedding(6080, 1024)` |
| `penalty8` | weighted MorphBPE, crossing penalty 8 @ 6,080 | fresh `nn.Embedding(6080, 1024)` |
...
- `build_colab_bundle.py` -> `data/bundle/{train,dev,test}.jsonl` + `meta.json`.
  Each row carries the Kapampangan side pre-tokenised by **both** MorphBPE
  artifacts (`morphbpe_ids`, `penalty8_ids`, ids in [0, 6080), `<s>`/`</s>`
  included), so the notebook needs no project code.

## Run

`notebooks/phase5-nllb-morphbpe-finetune.ipynb` -- upload the four bundle
files, `Run all`, T4 GPU, ~2-2.5 h (10 runs, results written incrementally
so a disconnect is recoverable). Emits `phase5-results.json` (per-run +
mean/std across seeds). See the notebook's own summary before running.
